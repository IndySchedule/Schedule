...
assert(!/localStorage\.setItem\([^\n]*(calendarUrl|assignment)|indexedDB/i.test(assignmentsSource), 'Schoology calendar URLs and assignment completion do not use persistent browser storage');
...
assert(assignmentsSource.includes("empty.textContent = 'Nothing due today'") && assignmentsSource.includes('const canShow = !!authUser() && state.connectionChecked && state.connected'), 'Dashboard Due Today has a connected empty state and stays hidden when unavailable');
...
assert(authSource.includes('indyReleaseNotice_v1_4_0: localStorage.getItem'), 'The 1.4.0 release-notice dismissal is included in account preference sync');
...
assert(indexSource.includes('Version 1.4.0') && indexSource.includes('v1.4.0'), 'About and release notes identify the current 1.4.0 version');
...
assert(indexSource.includes('id="release-notice-backdrop"') && indexSource.includes('Version 1.4.0'), 'Returning users receive the 1.4.0 release notice');
assert(indexSource.includes("const storageKey = 'indyReleaseNotice_v1_4_0'") && indexSource.includes("localStorage.setItem('indyReleaseNotice_v1_4_0', 'true')"), 'The 1.4.0 release notice uses a new one-time dismissal key');
...
assert(indexSource.includes('What’s new in 1.4.0') && indexSource.includes('Schoology Assignments') && indexSource.includes('Due today at a glance') && indexSource.includes('Feedback is easier too'), 'The 1.4.0 release notice explains assignments, the dashboard summary, and feedback');
...
assertEqual(JSON.parse(readFile('package.json')).version, '1.4.0', 'Package metadata identifies version 1.4.0');
assert([privacySource, termsSource].every((source) => source.includes('Version 1.4.0')), 'Privacy and Terms identify the current 1.4.0 version');
assertEqual((indexSource.match(/<div class="wn-entry(?: current-release)?">/g) || []).length, 14, 'What’s New includes the initial release and thirteen focused updates');
