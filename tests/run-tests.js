...
assert(authSource.includes('indyReleaseNotice_v1_3_5: localStorage.getItem'), 'The 1.3.5 release-notice dismissal is included in account preference sync');
...
assert(indexSource.includes('Version 1.3.5') && indexSource.includes('v1.3.5'), 'About and release notes identify the current 1.3.5 version');
...
assert(indexSource.includes('clearer names for color controls') && indexSource.includes('sync-details view'), 'The 1.3.5 notes explain the accessibility and sync improvements');
assert(indexSource.includes('id="release-notice-backdrop"') && indexSource.includes('Version 1.3.5'), 'Returning users receive the 1.3.5 release notice');
assert(indexSource.includes("const storageKey = 'indyReleaseNotice_v1_3_5'") && indexSource.includes("localStorage.setItem('indyReleaseNotice_v1_3_5', 'true')"), 'The 1.3.5 release notice uses a new one-time dismissal key');
...
assert(indexSource.includes('Accessibility and clearer sync') && indexSource.includes('Safer saved settings') && indexSource.includes('See all updates'), 'The release notice summarizes 1.3.4 and 1.3.5');
...
assertEqual(JSON.parse(readFile('package.json')).version, '1.3.5', 'Package metadata identifies version 1.3.5');
assert([privacySource, termsSource].every((source) => source.includes('Version 1.3.5')), 'Privacy and Terms identify the current 1.3.5 version');
assertEqual((indexSource.match(/<div class="wn-entry(?: current-release)?">/g) || []).length, 13, 'What’s New includes the initial release and twelve focused updates');
