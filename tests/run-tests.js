...
assert(authSource.includes('indyReleaseNotice_v1_3_4: localStorage.getItem'), 'The 1.3.4 release-notice dismissal is included in account preference sync');
...
assert(indexSource.includes('Version 1.3.4') && indexSource.includes('v1.3.4'), 'About and release notes identify the current 1.3.4 version');
...
assert(indexSource.includes('Added stricter checks so saved settings stay in the right format') && indexSource.includes('Removed an internal debugging file'), 'The 1.3.4 notes explain the safety cleanup in everyday language');
assert(indexSource.includes('id="release-notice-backdrop"') && indexSource.includes('Version 1.3.4'), 'Returning users receive the 1.3.4 release notice');
assert(indexSource.includes("const storageKey = 'indyReleaseNotice_v1_3_4'") && indexSource.includes("localStorage.setItem('indyReleaseNotice_v1_3_4', 'true')"), 'The 1.3.4 release notice uses a new one-time dismissal key');
...
assert(indexSource.includes('Safer saved settings') && indexSource.includes('See all updates'), 'The release notice summarizes 1.3.4 and links to the full history');
...
assertEqual(JSON.parse(readFile('package.json')).version, '1.3.4', 'Package metadata identifies version 1.3.4');
assert([privacySource, termsSource].every((source) => source.includes('Version 1.3.4')), 'Privacy and Terms identify the current 1.3.4 version');
assertEqual((indexSource.match(/<div class="wn-entry(?: current-release)?">/g) || []).length, 12, 'What’s New includes the initial release and eleven focused updates');
