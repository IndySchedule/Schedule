...
assert(['refresh', 'cover', 'shuffle', 'rename', 'remove'].every((action) => musicPlayerSource.includes(`data-playlist-action="${action}"`)), 'Music Player exposes all playlist menu actions');
assert(musicPlayerSource.includes('function choosePlaylistCover') && musicPlayerSource.includes("requestPermission({ mode: 'readwrite' })"), 'Music Player requests local folder editing permission before changing playlist artwork');
...
assert(musicPlayerSource.includes("indexedDB.open('local-music-player'") && musicPlayerSource.includes('restoreRememberedLibrary'), 'Music Player can locally restore an already-authorized folder after a page reload');
assert(musicPlayerSource.includes('savePlaybackState') && musicPlayerSource.includes('restorePlaybackState') && musicPlayerSource.includes("localMusic_playback"), 'Music Player restores the active local song and position after an unexpected reload');
...
assert(indexSource.indexOf('id="assignments-toggle"') < indexSource.indexOf('id="today-toggle"'), 'Assignments appears before Today at Indy in the dashboard header');
...
assert(assignmentsSource.includes("const SCHOOLOGY_API_BASE = 'https://schoology-fetcher.netlify.app'") && assignmentsSource.includes(': SCHOOLOGY_API_BASE;'), 'Production Schoology API requests use the Netlify relay');
assert(!assignmentsSource.includes('schoology-fetcher.bradyblackwell2009.workers.dev') && !assignmentsSource.includes('workers.dev'), 'Browser-facing assignments code contains no direct Cloudflare Worker URL');
assert(assignmentsSource.includes("'http://127.0.0.1:8787'") && assignmentsSource.includes("['localhost', '127.0.0.1']"), 'Localhost assignments use the local Worker alongside Firebase emulators');
assert(["'/api/schoology/connect'", "'/api/schoology/status'", "'/api/schoology/disconnect'", "'/api/assignments'"].every((route) => assignmentsSource.includes(route)), 'Schoology connection and assignment requests preserve their existing API routes');
assert(assignmentsSource.includes('`/api/assignments/${encodeURIComponent(id)}/complete`') && assignmentsSource.includes("method: next ? 'POST' : 'DELETE'"), 'Assignment completion preserves its authenticated POST and DELETE endpoint');
assert(assignmentsSource.includes('auth.onAuthStateChanged((user)') && assignmentsSource.includes("window.addEventListener('indy-firebase-ready'"), 'Assignments follow restored and emulator Firebase authentication state directly');
assert(readFile('auth.js').includes('checkLocalFirebaseEmulators') && readFile('auth.js').includes('Auth on port 9099 and Firestore on port 8080'), 'Local Firebase startup reports one actionable emulator message instead of initializing into repeated SDK failures');
assert(!/localStorage\.setItem\([^\n]*(calendarUrl|assignmentId|completed)|indexedDB/i.test(assignmentsSource), 'Schoology calendar URLs and assignment completion do not use persistent browser storage');
...
assert(assignmentsSource.includes("item.course || 'Schoology'") && !assignmentsSource.includes("item.course || 'Schoology calendar'"), 'Assignment cards use concise source copy when an ICS course is unavailable');
assert(indexSource.includes('id="assignments-connect-link"') && assignmentsSource.includes(".nav-item[data-target=\"schoology\"]"), 'Disconnected Assignments view links directly to Schoology Calendar settings');
assert(assignmentsSource.includes("badge.textContent = '1'") && assignmentsSource.includes('state.connectionPromptSeen = true'), 'Disconnected signed-in users receive a one-time Assignments connection badge');
assert(readFile('auth.js').includes("'indySchoologyPromptSeen_v1'") && assignmentsSource.includes("localStorage.setItem('indySchoologyPromptSeen_v1', 'true')") && assignmentsSource.includes('scheduleUserSettingsSave(0)'), 'Opening Assignments syncs the connection-prompt dismissal through Firestore settings');
assert(indexSource.includes('Connect Schoology iCalendar') && assignmentsSource.includes('Connect your Schoology iCalendar to see upcoming assignments.'), 'Disconnected assignment prompts clearly name the Schoology iCalendar connection');
assert(assignmentsSource.includes('Sign in to view assignments') && assignmentsSource.includes('Assignments require an Indy Schedule account'), 'Signed-out Assignments view explains that authentication is required instead of claiming there are no events');
assert(indexSource.includes('https://wcschools.schoology.com/settings/account#:~:text=Share%20Your%20Schoology%20Calendar'), 'Schoology setup links directly to the WCS calendar-sharing account section');
assert(indexSource.includes('target="_blank" rel="noopener noreferrer" aria-label="Open Schoology Calendar Settings in a new tab"'), 'Schoology settings link opens in a new tab with safe and accessible link attributes');
assert(['Open Schoology Calendar Settings', 'Copy your iCal link', 'Paste it here'].every((label) => indexSource.includes(label)), 'Disconnected Schoology state presents the complete guided three-step setup');
assert(indexSource.includes('webcal://wcschools.schoology.com/...') && indexSource.includes('id="schoology-connect-form"'), 'Guided Step 3 preserves the existing calendar connection form');
assert(indexSource.includes('id="schoology-replace-button"') && assignmentsSource.includes("$('schoology-disconnected').hidden = false"), 'Replace Calendar reuses the guided disconnected setup');
assert(indexSource.includes('Your assignments will refresh automatically when you open Assignments.') && indexSource.includes('id="schoology-disconnect-button"'), 'Connected Schoology state retains its controls with clearer refresh copy');
assert(assignmentsStyles.includes('.schoology-setup-step') && assignmentsStyles.includes('.schoology-open-settings:focus-visible') && assignmentsStyles.includes('@media (max-width:700px)'), 'Guided Schoology setup includes responsive and keyboard-focus styling');
assert(indexSource.includes('class="schoology-url-help"') && assignmentsStyles.includes('grid-template-columns:minmax(0, 1fr) auto'), 'Schoology Step 3 aligns its calendar field and Connect action in a clean responsive row');
assert(indexSource.includes('class="assignments-checkbox"') && assignmentsStyles.includes('.assignments-filter input:checked + .assignments-checkbox'), 'Show-completed control uses the active dashboard palette');
assert(assignmentsStyles.includes('.assignment-count-badge[hidden]') && assignmentsStyles.includes('display:none !important'), 'Assignments badge remains visually hidden after its intro is read or its count reaches zero');
assert(assignmentsStyles.includes('@keyframes assignments-view-enter') && assignmentsStyles.includes('.assignments-view:not([hidden])') && assignmentsStyles.includes('animation:none'), 'Assignments opens with a subtle animation that respects reduced-motion preferences');
...
assert(indexSource.includes('id="dashboard-due-today"') && indexSource.indexOf('id="schedule"') < indexSource.indexOf('id="dashboard-due-today"'), 'Dashboard places Due Today directly beneath the schedule');
assert(assignmentsSource.includes('const today = state.assignments.filter((item) => !item.completed') && assignmentsSource.includes('today.slice(0, 3)'), 'Dashboard Due Today reuses assignment state and limits the compact list to three incomplete items');
assert(assignmentsSource.includes("empty.textContent = 'Nothing due today'") && assignmentsSource.includes('dashboardDueTodayEnabled() && !!authUser() && state.connectionChecked && state.connected'), 'Dashboard Due Today has a connected empty state and stays hidden when unavailable or disabled');
assert(assignmentsSource.includes("more.dataset.openAssignments = ''") && assignmentsSource.includes("event.target.closest('[data-open-assignments]')"), 'Due Today rows and overflow action open the full Assignments view');
assert(assignmentsStyles.includes('.dashboard-due-row:focus-visible') && assignmentsStyles.includes('.dashboard-due-today[hidden]'), 'Dashboard Due Today provides keyboard focus styling and a reliable hidden state');
assert(indexSource.includes('id="show-due-today-assignments"') && assignmentsSource.includes("localStorage.getItem('showDueTodayAssignments') !== 'false'"), 'Schoology settings provide a default-on Due Today dashboard switch');
assert(assignmentsSource.includes("localStorage.setItem('showDueTodayAssignments'") && assignmentsSource.includes('scheduleUserSettingsSave(0)'), 'The Due Today dashboard preference saves locally and syncs to the account');
assert(readFile('auth.js').includes("'showDueTodayAssignments'") && readFile('firestore.rules').includes("settings.get('showDueTodayAssignments', 'true')"), 'The Due Today preference is included in validated Firestore settings');
...
assert(authSource.includes('indyReleaseNotice_v1_5_0: localStorage.getItem'), 'The 1.5.0 release-notice dismissal is included in account preference sync');
...
assert(indexSource.includes('Version 1.5.0') && indexSource.includes('v1.5.0'), 'About and release notes identify the current 1.5.0 version');
...
assert(indexSource.includes('id="release-notice-backdrop"') && indexSource.includes('Version 1.5.0'), 'Returning users receive the 1.5.0 release notice');
assert(indexSource.includes("const storageKey = 'indyReleaseNotice_v1_5_0'") && indexSource.includes("localStorage.setItem('indyReleaseNotice_v1_5_0', 'true')"), 'The 1.5.0 release notice uses a new one-time dismissal key');
...
assert(indexSource.includes('What’s new in 1.5.0') && indexSource.includes('Schoology Assignments') && indexSource.includes('Due today at a glance') && indexSource.includes('Feedback is easier too'), 'The 1.5.0 release notice explains assignments, the dashboard summary, and feedback');
...
assertEqual(JSON.parse(readFile('package.json')).version, '1.5.0', 'Package metadata identifies version 1.5.0');
assert([privacySource, termsSource].every((source) => source.includes('Version 1.5.0')), 'Privacy and Terms identify the current 1.5.0 version');
assertEqual((indexSource.match(/<div class="wn-entry(?: current-release)?">/g) || []).length, 15, 'What’s New includes the initial release and fourteen focused updates');
...
assert(indexSource.includes('aria-label="Primary page background color"') && indexSource.includes('aria-label="Surface cards and schedule color"'), 'Custom color controls have explicit accessible names');
assert(indexSource.includes('id="settings-sync-detail"') && authSource.includes('Sync revision'), 'Settings include an expandable sync status detail');
